...
def selfcheck(v):
    ...
    # And that AES-GCM here is the same AES-GCM wire_v3 pinned, so the frame
    # vectors below are not the first time this library is trusted.
...
# The same bytes as C, emitted rather than transcribed. A vector copied by hand
# is a vector that can disagree with the one the far side parses.
...
def main():
    ...
    # Hub term first, in every concatenation. Z1 authenticates the hub - only
    ...
    # Struct fields are LITTLE-endian, because the structs in
    # radio_protocol.h are the contract and both cores are little-endian. The
    # crypto inputs above are BIG-endian. The two rules coexist and the whole
    # frames are emitted below so nobody has to infer which applies where.
...
"""
    ...
    print()
    # A key that ignored the new freshness would silently reproduce pair_v1 and
    # look like a successful port. Asserted rather than eyeballed.
